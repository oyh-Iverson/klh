...
+ function($) {
    ...
    //閲嶇疆scrollLeft鍜宻crollRight
    (function() {
        $.extend($.fn, {
            scrollTop: function(top, dur) {
                if (!this.length) return;
                var scroller = this.data('scroller');
                if (scroller && scroller.scroller) { //js婊氬姩
    ...
    var Scroller = function(pageContent, _options) {
        ...
        //auto鐨則ype,绯荤粺鐗堟湰鐨勫皬浜�4.4.0鐨勫畨鍗撹澶囧拰绯荤粺鐗堟湰灏忎簬6.0.0鐨刬os璁惧锛屽惎鐢╦s鐗堢殑iscoll
        ...
        if (useJSScroller) {
            ...
            if ($pageContent.hasClass('pull-to-refresh-content')) {
                //鍥犱负iscroll 褰撻〉闈㈤珮搴︿笉瓒� 100% 鏃舵棤娉曟粦鍔紝鎵€浠ユ棤娉曡Е鍙戜笅鎷夊姩浣滐紝杩欓噷鏀瑰姩涓€涓嬮珮搴�
                //鍖哄垎鏄惁鏈�.bar瀹瑰櫒锛屽鏈夛紝鍒檆ontent鐨則op:0锛屾棤鍒檆ontent鐨則op:-2.2rem,杩欓噷鍙�2.2rem鐨勬渶澶у€硷紝杩�60
            ...
            //js婊氬姩妯″紡锛岀敤transform绉诲姩鍐呭鍖轰綅缃紝浼氬鑷磃ixed澶辨晥锛岃〃鐜扮被浼糰bsolute銆傚洜姝ょ鐢╰ransform妯″紡
            var useTransform = $pageContent.find('.fixed-tab').length === 0;
            var options = {
                probeType: 1,
                mouseWheel: true,
                //瑙ｅ喅瀹夊崜js妯″紡涓嬶紝鍒锋柊婊氬姩鏉″悗缁戝畾鐨勪簨浠朵笉鍝嶅簲锛屽chrome鍐呮牳娴忚鍣ㄨ缃甤lick:true
                click: $.device.androidChrome,
                useTransform: useTransform,
                //js妯″紡涓嬪厑璁告粴鍔ㄦ潯妯悜婊氬姩锛屼絾鏄渶瑕佹敞鎰忥紝婊氬姩瀹规槗瀹藉害蹇呴』澶т簬灞忓箷瀹藉害婊氬姩鎵嶇敓鏁�
                scrollX: true
            };
            if (ptr) {
                options.ptr = true;
            ...
            //濡傛灉鐢╦s婊氬姩鏉★紝鐢╰ransform璁＄畻鍐呭鍖轰綅缃紝position锛歠ixed灏嗗疄鏁堛€傝嫢鏈�.fixed-tab锛屽己鍒朵娇鐢╪ative婊氬姩鏉★紱澶囬€夋柟妗堬紝鐣ョ矖鏆�
            ...
            //濡傛灉椤甸潰鏈韩宸茬粡杩涜浜嗗師鐢熸粴鍔紝閭ｄ箞鎶婅繖涓粴鍔ㄦ崲鎴怞S鐨勬粴鍔�
    ...
    Scroller.prototype = {
        ...
        _bindEventToDomWhenJs: function() {
            ...
            if (this.scroller) {
                ...
                //TODO: 瀹炵幇native鐨剆crollStart鍜宻crollEnd
            }
        },
        scrollTop: function(top, dur) {
            if (this.scroller) {
    ...
    function Plugin(option) {
        ...
        this.each(function() {
            ...
            //濡傛灉 scroller 娌℃湁琚垵濮嬪寲锛屽scroller 杩涜鍒濆鍖杛
            if (!data) {
                //鑾峰彇data-api鐨�
    ...
    //缁熶竴鐨勬帴鍙�,甯︽湁 .javascript-scroll 鐨刢ontent 杩涜鍒锋柊
    ...
    //鍏ㄥ眬鍒濆鍖栨柟娉曪紝浼氬椤甸潰涓婄殑 [data-toggle="scroller"]锛�.content. 杩涜婊氬姩鏉″垵濮嬪寲
    ...
    $.getScroller = function(content) {
        //浠ュ墠榛樿鍙兘鏈変竴涓棤闄愭粴鍔紝鍥犳infinitescroll閮芥槸鍔犲湪content涓婏紝鐜板湪鍏佽閲岄潰鏈夊涓紝鍥犳瑕佸垽鏂埗鍏冪礌鏄惁鏈塩ontent
...
+function ($) {
    ...
    //a鏍囩涓婄殑click浜嬩欢锛屽湪iscroll涓嬪搷搴旀湁闂
...
+function ($) {
    ...
    $.initFixedTab = function(){
        ...
        $('.fixed-tab').fixedTab();//榛樿{offset: 0}
    };
...
+ function($) {
    ...
    $.pullToRefreshDone = function(container) {
        $(window).scrollTop(0);//瑙ｅ喅寰俊涓嬫媺鍒锋柊椤堕儴娑堝け鐨勯棶棰�
        container = $(container);
        if (container.length === 0) container = $('.pull-to-refresh-content.refreshing');
...
+ function($) {
    ...
    $.initInfiniteScroll = function(pageContainer) {
        ...
        //濡傛灉鏄《閮ㄦ棤闄愬埛鏂帮紝瑕佸皢婊氬姩鏉″垵濮嬪寲浜庢渶涓嬬
 ...
 * 璺敱
 *
 * 璺敱鍔熻兘灏嗘帴绠￠〉闈㈢殑閾炬帴鐐瑰嚮琛屼负锛屾渶鍚庤揪鍒板姩鐢诲垏鎹㈢殑鏁堟灉锛屽叿浣撳涓嬶細
 *  1. 閾炬帴瀵瑰簲鐨勬槸鍙︿竴涓〉闈紝閭ｄ箞鍒欏皾璇� ajax 鍔犺浇锛岀劧鍚庢妸鏂伴〉闈㈤噷鐨勭鍚堢害瀹氱殑缁撴瀯鎻愬彇鍑烘潵锛岀劧鍚庡仛鍔ㄧ敾鍒囨崲锛涘鏋滄病娉� ajax 鎴栫粨鏋勪笉绗﹀悎锛岄偅涔堝垯鍥為€€涓烘櫘閫氱殑椤甸潰璺宠浆
 *  2. 閾炬帴鏄綋鍓嶉〉闈㈢殑閿氱偣锛屽苟涓旇閿氱偣瀵瑰簲鐨勫厓绱犲瓨鍦ㄤ笖绗﹀悎璺敱绾﹀畾锛岄偅涔堝垯鎶婅鍏冪礌鍋氬姩鐢诲垏鍏�
 *  3. 娴忚鍣ㄥ墠杩涘悗閫€锛坔istory.forward/history.back锛夋椂锛屼篃浣跨敤鍔ㄧ敾鏁堟灉
 *  4. 濡傛灉閾炬帴鏈� back 杩欎釜 class锛岄偅涔堝垯蹇界暐涓€鍒囷紝鐩存帴璋冪敤 history.back() 鏉ュ悗閫€
 *
 * 璺敱鍔熻兘榛樿寮€鍚紝濡傛灉闇€瑕佸叧闂矾鐢卞姛鑳斤紝閭ｄ箞鍦� zepto 涔嬪悗锛宮sui 鑴氭湰涔嬪墠璁剧疆 $.config.router = false 鍗冲彲锛坕ntro.js 涓細 extend 鍒� $.smConfig 涓級銆�
 *
 * 鍙互璁剧疆 $.config.routerFilter 鍑芥暟鏉ヨ缃綋鍓嶇偣鍑婚摼鎺ユ槸鍚︿娇鐢ㄨ矾鐢卞姛鑳斤紝瀹炲弬鏄� a 閾炬帴鐨� zepto 瀵硅薄锛涜繑鍥� false 琛ㄧず涓嶄娇鐢� router 鍔熻兘銆�
 *
 * ajax 杞藉叆鏂扮殑鏂囨。鏃讹紝骞朵笉浼氭墽琛岄噷闈㈢殑 js銆傚埌鐩墠涓烘锛屽湪寮€鍚矾鐢卞姛鑳芥椂锛屽缓璁殑鍋氭硶鏄細
 *  鎶婃墍鏈夐〉闈㈢殑 js 閮芥斁鍒板悓涓€涓剼鏈噷锛宩s 閲岄潰鐨勪簨浠剁粦瀹氫娇鐢ㄥ鎵樿€屼笉鏄洿鎺ョ殑缁戝畾鍦ㄥ厓绱犱笂锛堝洜涓哄姩鎬佸姞杞界殑椤甸潰鍏冪礌杩樹笉瀛樺湪锛夛紝鐒跺悗鎵€鏈夐〉闈㈤兘寮曠敤鐩稿悓鐨� js 鑴氭湰銆傞潪浜嬩欢绫诲彲浠ラ€氳繃鐩戞帶 pageInit 浜嬩欢锛屾牴鎹噷闈㈢殑 pageId 鏉ュ仛瀵瑰簲鍖哄埆澶勭悊銆�
 *
 * 濡傛灉鏈夐渶瑕�
 *
 * 瀵瑰鏆撮湶鐨勬柟娉�
 *  - load 锛堝師 loadPage 鏁堟灉涓€鑷�,浣嗗悗鑰呭凡鏍囪涓哄緟绉婚櫎锛�
 *  - forward
 *  - back
 *
 * 浜嬩欢
 * pageLoad* 绯诲垪鍦ㄥ彂鐢� ajax 鍔犺浇鏃舵墠浼氳Е鍙戯紱褰撴槸鍧楀垏鎹㈡垨宸茬紦瀛樼殑鎯呭喌涓嬶紝涓嶄細鍙戦€佽繖浜涗簨浠�
 *  - pageLoadCancel: 濡傛灉鍓嶄竴涓繕娌″姞杞藉畬,閭ｄ箞鍙栨秷骞跺彂閫佽浜嬩欢
 *  - pageLoadStart: 寮€濮嬪姞杞�
 *  - pageLodComplete: ajax complete 瀹屾垚
 *  - pageLoadError: ajax 鍙戠敓 error
 *  - pageAnimationStart: 鎵ц鍔ㄧ敾鍒囨崲鍓嶏紝瀹炲弬鏄� event锛宻ectionId 鍜� $section
 *  - pageAnimationEnd: 鎵ц鍔ㄧ敾瀹屾瘯锛屽疄鍙傛槸 event锛宻ectionId 鍜� $section
 *  - beforePageRemove: 鏂� document 杞藉叆涓斿姩鐢诲垏鎹㈠畬姣曪紝鏃х殑 document remove 涔嬪墠鍦� window 涓婅Е鍙戯紝瀹炲弬鏄� event 鍜� $pageContainer
 *  - pageRemoved: 鏂扮殑 document 杞藉叆涓斿姩鐢诲垏鎹㈠畬姣曪紝鏃х殑 document remove 涔嬪悗鍦� window 涓婅Е鍙�
 *  - beforePageSwitch: page 鍒囨崲鍓嶏紝鍦� pageAnimationStart 鍓嶏紝beforePageSwitch 涔嬪悗浼氬仛涓€浜涢澶栫殑澶勭悊鎵嶈Е鍙� pageAnimationStart
 *  - pageInitInternal: 锛堢粡 init.js 澶勭悊鍚庯紝瀵瑰鏄� pageInit锛夌揣璺熺潃鍔ㄧ敾瀹屾垚鐨勪簨浠讹紝瀹炲弬鏄� event锛宻ectionId 鍜� $section
 *
 * 鏈
 *  - 鏂囨。锛坉ocument锛夛紝涓嶅甫 hash 鐨� url 鍏宠仈鐫€鐨勫簲绛� html 缁撴瀯
 *  - 鍧楋紙section锛夛紝涓€涓枃妗ｅ唴鏈夋寚瀹氬潡鏍囪瘑鐨勫厓绱�
 *
 * 璺敱瀹炵幇绾﹀畾
 *  - 姣忎釜鏂囨。鐨勯渶瑕佸睍绀虹殑鍐呭蹇呴渶浣嶄簬鎸囧畾鐨勬爣璇嗭紙routerConfig.sectionGroupClass锛夌殑鍏冪礌閲岄潰锛岄粯璁ゆ槸: div.page-group 锛堟敞鎰�,濡傛灉鏀瑰彉杩欎釜闇€瑕佸悓鏃舵敼鍙� less 涓殑鍛藉悕锛�
 *  - 姣忎釜鍧楀繀闇€甯︽湁鎸囧畾鐨勫潡鏍囪瘑锛坮outerConfig.pageClass锛夛紝榛樿鏄� .page
 *
 *  鍗筹紝浣跨敤璺敱鍔熻兘鐨勬瘡涓€涓枃妗ｅ簲褰撴槸涓嬮潰杩欐牱鐨勭粨鏋勶紙鐪佺暐 <body> 绛夛級:
 *      <div class="page-group">
 *          <div class="page">xxx</div>
 *          <div class="page">yyy</div>
 *      </div>
 *
 * 鍙︼紝姣忎竴涓潡閮藉簲褰撴湁涓€涓敮涓€鐨� ID锛岃繖鏍锋墠鑳介€氳繃 #the-id 鐨勫舰寮忔潵鍒囨崲瀹氫綅銆�
 * 褰撲竴涓潡娌℃湁 id 鏃讹紝濡傛灉鏄涓€涓殑榛樿鐨勯渶瑕佸睍绀虹殑鍧楋紝閭ｄ箞浼氱粰鍏舵坊鍔犱竴涓殢鏈虹殑 id锛涘惁鍒欙紝娌℃湁 id 鐨勫潡灏嗕笉浼氳鍒囨崲灞曠ず銆�
 *
 * 閫氳繃 history.state/history.pushState 浠ュ強鐢� sessionStorage 鏉ヨ褰曞綋鍓� state 浠ュ強鏈€澶х殑 state id 鏉ヨ緟鍔╁墠杩涘悗閫€鐨勫垏鎹㈡晥鏋滐紝鎵€浠ュ湪涓嶆敮鎸� sessionStorage 鐨勬儏鍐典笅锛屽皢涓嶅紑鍚矾鐢卞姛鑳姐€�
 *
 * 涓轰簡瑙ｅ喅 ajax 杞藉叆椤甸潰瀵艰嚧閲嶅 ID 浠ュ強閲嶅 popup 绛夊姛鑳斤紝涓婇潰绾﹀畾浜嗕娇鐢ㄨ矾鐢卞姛鑳界殑鎵€鏈夊彲灞曠ず鍐呭閮藉繀闇€浣嶄簬鎸囧畾鍏冪礌鍐呫€備粠鑰屽彲浠ュ湪杩涜鏂囨。闂村垏鎹㈡椂鍙互杩涜涓や釜鏂囨。鐨勬暣浣撶Щ鍔紝鍒囨崲瀹屾瘯鍚庡啀鎶婂墠涓€涓枃妗ｇ殑鍐呭浠庨〉闈箣闂寸Щ闄ゃ€�
 *
 * 榛樿鍦拌繃婊や簡閮ㄥ垎鍗忚鐨勯摼鎺ワ紝鍖呮嫭 tel:, javascript:, mailto:锛岃繖浜涢摼鎺ュ皢涓嶄細浣跨敤璺敱鍔熻兘銆傚鏋滄湁鏇村鐨勮嚜瀹氫箟鎺у埗闇€姹傦紝鍙互鍦� $.config.routerFilter 瀹炵幇
 *
 * 娉�: 浠� _ 寮€澶寸殑鍑芥暟鏍囨槑鐢ㄤ簬姝ゅ鍐呴儴浣跨敤锛屽彲鏍规嵁闇€瑕侀殢鏃堕噸鏋勫彉鏇达紝涓嶅澶栫‘淇濆吋瀹规€с€�
 *
 */
+function($) {
    'use strict';
    ...
    var EVENTS = {
        pageLoadStart: 'pageLoadStart', // ajax 寮€濮嬪姞杞芥柊椤甸潰鍓�
        pageLoadCancel: 'pageLoadCancel', // 鍙栨秷鍓嶄竴涓� ajax 鍔犺浇鍔ㄤ綔鍚�
        pageLoadError: 'pageLoadError', // ajax 鍔犺浇椤甸潰澶辫触鍚�
        pageLoadComplete: 'pageLoadComplete', // ajax 鍔犺浇椤甸潰瀹屾垚鍚庯紙涓嶈鎴愬姛涓庡惁锛�
        pageAnimationStart: 'pageAnimationStart', // 鍔ㄧ敾鍒囨崲 page 鍓�
        pageAnimationEnd: 'pageAnimationEnd', // 鍔ㄧ敾鍒囨崲 page 缁撴潫鍚�
        beforePageRemove: 'beforePageRemove', // 绉婚櫎鏃� document 鍓嶏紙閫傜敤浜庨潪鍐呰仈 page 鍒囨崲锛�
        pageRemoved: 'pageRemoved', // 绉婚櫎鏃� document 鍚庯紙閫傜敤浜庨潪鍐呰仈 page 鍒囨崲锛�
        beforePageSwitch: 'beforePageSwitch', // page 鍒囨崲鍓嶏紝鍦� pageAnimationStart 鍓嶏紝beforePageSwitch 涔嬪悗浼氬仛涓€浜涢澶栫殑澶勭悊鎵嶈Е鍙� pageAnimationStart
        pageInit: 'pageInitInternal' // 鐩墠鏄畾涔変负涓€涓� page 鍔犺浇瀹屾瘯鍚庯紙瀹為檯鍜� pageAnimationEnd 绛夊悓锛�
    };

    var Util = {
        /**
         * 鑾峰彇 url 鐨� fragment锛堝嵆 hash 涓幓鎺� # 鐨勫墿浣欓儴鍒嗭級
         *
         * 濡傛灉娌℃湁鍒欒繑鍥炵┖瀛楃涓�
         * 濡�: http://example.com/path/?query=d#123 => 123
         *
        ...
        /**
         * 鑾峰彇涓€涓摼鎺ョ浉瀵逛簬褰撳墠椤甸潰鐨勭粷瀵瑰湴鍧€褰㈠紡
         *
         * 鍋囪褰撳墠椤甸潰鏄� http://a.com/b/c
         * 閭ｄ箞鏈変互涓嬫儏鍐�:
         * d => http://a.com/b/d
         * /e => http://a.com/e
         * #1 => http://a.com/b/c#1
        ...
        /**
         * 鑾峰彇涓€涓� url 鐨勫熀鏈儴鍒嗭紝鍗充笉鍖呮嫭 hash
         *
         * @param {String} url url
         * @returns {String}
         */
        ...
        /**
         * 鎶婁竴涓瓧绗︿覆鐨� url 杞负涓€涓彲鑾峰彇鍏� base 鍜� fragment 绛夌殑瀵硅薄
         *
         * @param {String} url url
         * @returns {UrlObject}
         */
        ...
        /**
         * 鍒ゆ柇娴忚鍣ㄦ槸鍚︽敮鎸� sessionStorage锛屾敮鎸佽繑鍥� true锛屽惁鍒欒繑鍥� false
         * @returns {Boolean}
         */
    ...
    var routerConfig = {
        ...
        // 琛ㄧず鏄綋鍓� page 鐨� class
        curPageClass: 'page-current',
        // 鐢ㄦ潵杈呭姪鍒囨崲鏃惰〃绀� page 鏄� visible 鐨�,
        // 涔嬫墍浠ヤ笉鐢� curPageClass锛屾槸鍥犱负 page-current 宸茶璧嬩簣浜嗐€屽綋鍓� page銆嶈繖涓€鍚箟鑰屼笉浠呬粎鏄� display: block
        // 骞朵笖锛屽埆鐨勫湴鏂瑰凡缁忎娇鐢ㄤ簡锛屾墍浠ヤ笉鏂逛究鍋氬彉鏇达紝鏁呮柊澧炰竴涓�
        visiblePageClass: 'page-visible',
        // 琛ㄧず鏄� page 鐨� class锛屾敞鎰忥紝浠呮槸鏍囧織 class锛岃€屼笉鏄墍鏈夌殑 class
        pageClass: 'page'
    };
     ...
     *
     * - 鎶婂綋鍓嶆枃妗ｅ唴瀹圭紦瀛樿捣鏉�
     * - 鏌ユ壘榛樿灞曠ず鐨勫潡鍐呭锛屾煡鎵鹃『搴忓涓�
     *      1. id 鏄� url 涓殑 fragment 鐨勫厓绱�
     *      2. 鏈夊綋鍓嶅潡 class 鏍囪瘑鐨勭涓€涓厓绱�
     *      3. 绗竴涓潡
     * - 鍒濆椤甸潰 state 澶勭悊
     *
     * @private
    ...
    Router.prototype._init = function() {
        ...
        if ($curVisibleSection.length &&
            ...
            // 鍦� router 鍒� inner page 鐨勬儏鍐典笅锛屽埛鏂帮紙鎴栬€呯洿鎺ヨ闂閾炬帴锛�
            // 鐩存帴鍒囨崲 class 浼氭湁銆岄棯銆嶇殑鐜拌薄,鎴栬鍙互閲囩敤 animateSection 鏉ュ噺缂撲竴涓�
        ...
        // 鏂拌繘鍏ヤ竴涓娇鐢� history.state 鐩稿叧鎶€鏈殑椤甸潰鏃讹紝濡傛灉绗竴涓� state 涓� push/replace,
        // 閭ｄ箞鍦ㄥ悗閫€鍥炶椤甸潰鏃讹紝灏嗕笉瑙﹀彂 popState 浜嬩欢
     ...
     * 鍒囨崲鍒� url 鎸囧畾鐨勫潡鎴栨枃妗�
     *
     * 濡傛灉 url 鎸囧悜鐨勬槸褰撳墠椤甸潰锛岄偅涔堣涓烘槸鍒囨崲鍧楋紱
     * 鍚﹀垯鏄垏鎹㈡枃妗�
     *
     * @param {String} url url
     * @param {Boolean=} ignoreCache 鏄惁寮哄埗璇锋眰涓嶄娇鐢ㄧ紦瀛橈紝瀵� document 鐢熸晥锛岄粯璁ゆ槸 false
     */
     ...
     * 鍒囨崲鏄剧ず褰撳墠鏂囨。鍙︿竴涓潡
     *
     * 鎶婃柊鍧椾粠鍙宠竟鍒囧叆灞曠ず锛屽悓鏃朵細鎶婃柊鐨勫潡鐨勮褰曠敤 history.pushState 鏉ヤ繚瀛樿捣鏉�
     *
     * 濡傛灉宸茬粡鏄綋鍓嶆樉绀虹殑鍧楋紝閭ｄ箞涓嶅仛浠讳綍澶勭悊锛�
     * 濡傛灉娌″搴旂殑鍧楋紝閭ｄ箞蹇界暐銆�
     *
     * @param {String} sectionId 寰呭垏鎹㈡樉绀虹殑鍧楃殑 id
     * @private
     */
     ...
     * 杞藉叆鏄剧ず涓€涓柊鐨勬枃妗�
     *
     * - 濡傛灉鏈夌紦瀛橈紝閭ｄ箞鐩存帴鍒╃敤缂撳瓨鏉ュ垏鎹�
     * - 鍚﹀垯锛屽厛鎶婇〉闈㈠姞杞借繃鏉ョ紦瀛橈紝鐒跺悗鍐嶅垏鎹�
     *      - 濡傛灉瑙ｆ瀽澶辫触锛岄偅涔堢敤 location.href 鐨勬柟寮忔潵璺宠浆
     *
     * 娉ㄦ剰锛氫笉鑳藉湪杩欓噷浠ュ強鍏朵箣鍚庣敤 location.href 鏉� **璇诲彇** 鍒囨崲鍓嶇殑椤甸潰鐨� url锛�
     *     鍥犱负濡傛灉鏄� popState 鏃剁殑璋冪敤锛岄偅涔堟鏃� location 宸茬粡鏄� pop 鍑烘潵鐨� state 鐨勪簡
     *
     * @param {String} url 鏂扮殑鏂囨。鐨� url
     * @param {Boolean=} ignoreCache 鏄惁涓嶄娇鐢ㄧ紦瀛樺己鍒跺姞杞介〉闈�
     * @param {Boolean=} isPushState 鏄惁闇€瑕� pushState
     * @param {String=} direction 鏂版枃妗ｅ垏鍏ョ殑鏂瑰悜
     * @private
     */
    ...
    /**
     * 鍒╃敤缂撳瓨鏉ュ仛鍏蜂綋鐨勫垏鎹㈡枃妗ｆ搷浣�
     *
     * - 纭畾寰呭垏鍏ョ殑鏂囨。鐨勯粯璁ゅ睍绀� section
     * - 鎶婃柊鏂囨。 append 鍒� view 涓�
     * - 鍔ㄧ敾鍒囨崲鏂囨。
     * - 濡傛灉闇€瑕� pushState锛岄偅涔堟妸鏈€鏂扮殑鐘舵€� push 杩涘幓骞舵妸褰撳墠鐘舵€佹洿鏂颁负璇ョ姸鎬�
     *
     * @param {String} url 寰呭垏鎹㈢殑鏂囨。鐨� url
     * @param {Boolean} isPushState 鍔犺浇椤甸潰鍚庢槸鍚﹂渶瑕� pushState锛岄粯璁ゆ槸 true
     * @param {String} direction 鍔ㄧ敾鍒囨崲鏂瑰悜锛岄粯璁ゆ槸 DIRECTION.rightToLeft
     * @private
     */
    Router.prototype._doSwitchDocument = function(url, isPushState, direction) {
        ...
        // 纭畾涓€涓� document 灞曠ず section 鐨勯『搴�
        // 1. 涓� hash 鍏宠仈鐨� element
        // 2. 榛樿鐨勬爣璇嗕负 current 鐨� element
        // 3. 绗竴涓� section
        ...
        // prepend 鑰屼笉 append 鐨勭洰鐨勬槸閬垮厤 append 杩涘幓鏂扮殑 document 鍦ㄥ悗闈紝
        // 鍏堕噷闈㈢殑榛樿灞曠ず鐨�(.page-current) 鐨勯〉闈㈢洿鎺ュ氨瑕嗙洊浜嗗師鏄剧ず鐨勯〉闈紙鍥犱负閮芥槸 absolute锛�
    ...
    /**
     * 鍒ゆ柇涓や釜 url 鎸囧悜鐨勯〉闈㈡槸鍚︽槸鍚屼竴涓�
     *
     * 鍒ゆ柇鏂瑰紡: 濡傛灉涓や釜 url 鐨� base 褰㈠紡锛堜笉甯� hash 鐨勭粷瀵瑰舰寮忥級鐩稿悓锛岄偅涔堣涓烘槸鍚屼竴涓〉闈�
     *
     * @param {String} url
     ...
     *
     * @param {String} url url
     * @param {Object=} callback 鍥炶皟鍑芥暟閰嶇疆锛屽彲閫夛紝鍙互閰嶇疆 success\error 鍜� complete
     *      鎵€鏈夊洖璋冨嚱鏁扮殑 this 閮芥槸 null锛屽悇鑷疄鍙傚涓嬶細
    ...
    Router.prototype._loadDocument = function(url, callback) {
        ...
        this.xhr = $.ajax({
            ...
            success: $.proxy(function(data, status, xhr) {
                // 缁欏寘涓€灞� <html/>锛屼粠鑰屽彲浠ユ嬁鍒板畬鏁寸殑缁撴瀯
                var $doc = $('<html></html>');
                $doc.append(data);
    ...
    /**
     * 瀵逛簬 ajax 鍔犺浇杩涙潵鐨勯〉闈紝鎶婂叾缂撳瓨璧锋潵
     *
     * @param {String} url url
     * @param $doc ajax 杞藉叆鐨勯〉闈㈢殑 jq 瀵硅薄锛屽彲浠ョ湅鍋氭槸璇ラ〉闈㈢殑 $(document)
     * @private
     */
    ...
    /**
     * 鎶婁竴涓〉闈㈢殑鐩稿叧淇℃伅淇濆瓨鍒� this.cache 涓�
     *
     * 浠ラ〉闈㈢殑 baseUrl 涓� key,鑰� value 鍒欐槸涓€涓� DocumentCache
     *
     * @param {*} doc doc
     * @param {String} url url
     * @private
    ...
    /**
     * 浠� sessionStorage 涓幏鍙栦繚瀛樹笅鏉ョ殑銆屽綋鍓嶇姸鎬併€�
     *
     * 濡傛灉瑙ｆ瀽澶辫触锛岄偅涔堣涓哄綋鍓嶇姸鎬佹槸 null
     *
     * @returns {State|null}
     * @private
     */
    ...
    /**
     * 鎶婁竴涓姸鎬佽涓哄綋鍓嶇姸鎬侊紝淇濆瓨浠� sessionStorage 涓�
     *
     * @param {State} state
     * @private
     */
     ...
     * 鑾峰彇涓嬩竴涓� state 鐨� id
     *
     * 璇诲彇 sessionStorage 閲岀殑鏈€鍚庣殑鐘舵€佺殑 id锛岀劧鍚� + 1锛涘鏋滃師娌¤缃紝閭ｄ箞杩斿洖 1
     *
     * @returns {number}
     * @private
     */
    ...
    /**
     * 鎶� sessionStorage 閲岀殑鏈€鍚庣姸鎬佺殑 id 鑷姞 1
     *
     * @private
     */
    ...
    /**
     * 浠庝竴涓枃妗ｅ垏鎹负鏄剧ず鍙︿竴涓枃妗�
     *
     * @param $from 鐩墠鏄剧ず鐨勬枃妗�
     * @param $to 寰呭垏鎹㈡樉绀虹殑鏂版枃妗�
     * @param $visibleSection 鏂版枃妗ｄ腑灞曠ず鐨� section 鍏冪礌
     * @param direction 鏂版枃妗ｅ垏鍏ユ柟鍚�
     * @private
     */
    Router.prototype._animateDocument = function($from, $to, $visibleSection, direction) {
        ...
        $from.animationEnd(function() {
            ...
            // 绉婚櫎 document 鍓嶅悗锛屽彂閫� beforePageRemove 鍜� pageRemoved 浜嬩欢
        ...
        $to.animationEnd(function() {
            ...
            // 澶栧眰锛坕nit.js锛変腑浼氱粦瀹� pageInitInternal 浜嬩欢锛岀劧鍚庡椤甸潰杩涜鍒濆鍖�
            $visibleSection.trigger(EVENTS.pageInit, [sectionId, $visibleSection]);
        });
    };

    /**
     * 鎶婂綋鍓嶆枃妗ｇ殑灞曠ず section 浠庝竴涓� section 鍒囨崲鍒板彟涓€涓� section
     *
     * @param $from
     * @param $to
     * @param direction
     ...
     * 鍒囨崲鏄剧ず涓や釜鍏冪礌
     *
     * 鍒囨崲鏄€氳繃鏇存柊 class 鏉ュ疄鐜扮殑锛岃€屽叿浣撶殑鍒囨崲鍔ㄧ敾鍒欐槸 class 鍏宠仈鐨� css 鏉ュ疄鐜�
     *
     * @param $from 褰撳墠鏄剧ず鐨勫厓绱�
     * @param $to 寰呮樉绀虹殑鍏冪礌
     * @param direction 鍒囨崲鐨勬柟鍚�
     * @private
     */
    Router.prototype._animateElement = function($from, $to, direction) {
        // todo: 鍙€冭檻濡傛灉鍏ュ弬涓嶆寚瀹氾紝閭ｄ箞灏濊瘯璇诲彇 $to 鐨勫睘鎬э紝鍐嶆病鏈夊啀浣跨敤榛樿鐨�
        // 鑰冭檻璇诲彇鐐瑰嚮鐨勯摼鎺ヤ笂鎸囧畾鐨勬柟鍚�
    ...
    /**
     * popState 浜嬩欢鍏宠仈鐫€鐨勫悗閫€澶勭悊
     *
     * 鍒ゆ柇涓や釜 state 鍒ゆ柇鏄惁鏄睘浜庡悓涓€涓枃妗ｏ紝鐒跺悗鍋氬搴旂殑 section 鎴栨枃妗ｅ垏鎹紱
     * 鍚屾椂鍦ㄥ垏鎹㈠悗鎶婃柊 state 璁句负褰撳墠 state
     *
     * @param {State} state 鏂� state
     * @param {State} fromState 鏃� state
     * @private
     */
    ...
    /**
     * popState 浜嬩欢鍏宠仈鐫€鐨勫墠杩涘鐞�,绫讳技浜� _back锛屼笉鍚岀殑鏄垏鎹㈡柟鍚�
     *
     * @param {State} state 鏂� state
     * @param {State} fromState 鏃� state
     * @private
     ...
     * popState 浜嬩欢澶勭悊
     *
     * 鏍规嵁 pop 鍑烘潵鐨� state 鍜屽綋鍓� state 鏉ュ垽鏂槸鍓嶈繘杩樻槸鍚庨€€
     *
     * @param event
     * @private
     */
     ...
     * 椤甸潰杩涘叆鍒颁竴涓柊鐘舵€�
     *
     * 鎶婃柊鐘舵€� push 杩涘幓锛岃缃负褰撳墠鐨勭姸鎬侊紝鐒跺悗鎶� maxState 鐨� id +1銆�
     *
     * @param {String} url 鏂扮姸鎬佺殑 url
     * @param {String} sectionId 鏂扮姸鎬佷腑鏄剧ず鐨� section 鍏冪礌鐨� id
     * @private
     */
    ...
    function isInRouterBlackList($link) {
        ...
        //濡傛灉闈瀗oscheme褰㈠紡鐨勯摼鎺ワ紝涓斿崗璁笉鏄痟ttp(s)锛岄偅涔堣矾鐢变笉浼氬鐞嗚繖绫婚摼鎺�
    ...
    /**
     * 鑷畾涔夋槸鍚︽墽琛岃矾鐢卞姛鑳界殑杩囨护鍣�
     *
     * 鍙互鍦ㄥ閮ㄥ畾涔� $.config.routerFilter 鍑芥暟锛屽疄鍙傛槸鐐瑰嚮閾炬帴鐨� Zepto 瀵硅薄銆�
     *
     * @param $link 褰撳墠鐐瑰嚮鐨勯摼鎺ョ殑 Zepto 瀵硅薄
     * @returns {boolean} 杩斿洖 true 琛ㄧず鎵ц璺敱鍔熻兘锛屽惁鍒欎笉鍋氳矾鐢卞鐞�
     */
...
/**
 * @typedef {Object} UrlObject 瀛楃涓� url 杞负鐨勫璞�
 * @property {String} base url 鐨勫熀鏈矾寰�
 * @property {String} full url 鐨勫畬鏁寸粷瀵硅矾寰�
 * @property {String} origin 杞崲鍓嶇殑 url
 * @property {String} fragment url 鐨� fragment
 */

/**
 * @typedef {Object} DocumentCache
 * @property {*|HTMLElement} $doc 鐪嬪仛鏄� $(document)
 * @property {*|HTMLElement} $content $doc 閲岀殑 routerConfig.innerViewClass 鍏冪礌
 */
...
+function($) {
    ...
    $.initPage = function(page) {
        ...
        var $content = $page.hasClass('content') ?
                       $page :
                       $page.find('.content');
        $content.scroller();  //娉ㄦ剰婊氬姩鏉′竴瀹氳鏈€鍏堝垵濮嬪寲
    ...
    $(window).on('pageAnimationStart', function(event,id,page) {
        // 鍦ㄨ矾鐢卞垏鎹㈤〉闈㈠姩鐢诲紑濮嬪墠,涓轰簡鎶婁綅浜� .page 涔嬪鐨� popup 绛夐殣钘�,姝ゅ鍋氫簺澶勭悊
        $.closeModal();
        $.closePanel();
        // 濡傛灉 panel 鐨� effect 鏄� reveal 鏃�,浼间箮鏄� page 鐨勫姩鐢绘垨鍒殑鏍峰紡鍘熷洜瀵艰嚧浜� transitionEnd 鏃堕棿涓嶄細瑙﹀彂
        // 杩欓噷鏆備笖澶勭悊涓€涓�
        $('body').removeClass('panel-closing');
        $.allowPanelOpen = true;  
    });
    ...
    // safari 鍦ㄥ悗閫€鐨勬椂鍊欎細浣跨敤缂撳瓨鎶€鏈紝浣嗗疄鐜颁笂浼间箮瀛樺湪浜涢棶棰橈紝
    // 瀵艰嚧璺敱涓粦瀹氱殑鐐瑰嚮浜嬩欢涓嶄細姝ｅ父濡傛湡鐨勮繍琛岋紙log 鍜� debugger 閮芥病娉曡皟璇曪級锛�
    // 浠庤€屽悗缁殑璺宠浆绛夊畬鍏ㄤ贡浜嗗銆�
    // 鎵€浠ワ紝杩欓噷妫€娴嬪埌鏄� safari 鐨� cache 鐨勬儏鍐典笅锛屽仛涓€娆� reload
    // 娴嬭瘯璺緞(鍚庣紑 D 琛ㄧず鏄� document锛孍 琛ㄧず external锛屼笉浣跨敤璺敱璺宠浆锛夛細
    // 1. aD -> bDE
    // 2. back
    // 3. aD -> bD
...
+ function($) {
    "use strict";
    //瀹夊崜寰俊涓娇鐢╯crollfix浼氭湁闂锛屽洜姝ゅ彧鍦╥os涓娇鐢紝瀹夊崜鏈哄櫒鎸夌収鍘熸潵鐨勯€昏緫
