var app = new Vue({
	...
	mounted: function() {
		...
		if(isWeiXin()) {
			if(this.getQueryString('code') !== null && this.getQueryString('code') !== "") {
				...
				var uri = Util.baseUrl1 + "/dkdk/API/template/NetBlackziliao.html?referee="+referee;
	...
	methods: {
		...
		GoOk: function() {			
			var self = this;
			localStorage.setItem('phone',self.uerPhon);
			//alert(self.uerPhon);
			function isWeiXin() {
				var ua = window.navigator.userAgent.toLowerCase();
					if(ua.match(/MicroMessenger/i) == 'micromessenger') {
